...
* Posts
** DONE Being a PhD Student in the age of AI, part 1
:PROPERTIES:
:EXPORT_FILE_NAME: phd-ai-part-1
...
I default to L3, and the dial auto-adjusts: two failed attempts at the same step drops it a level, sailing through one bumps it up. A companion [[https://github.com/golanor/agent-skills/tree/main/english-writing-coach][English writing coach]] skill applies the same idea to prose — it diagnoses a problem by name and points at its neighborhood, but never rewrites the sentence for me, so I stay the author. Working this way, the friction is real, and so is the sense that the final product is mine.
